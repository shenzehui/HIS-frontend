<template>
  ...
  <div>
    <aside>
      ...
      <el-input size="mini" v-show="isshow" v-model="dmsRegistrationParam.invoiceNo" placeholder="发票号"
                style="width: 200px;margin-left:30px" class="filter-item"></el-input>
      <el-button v-show="isshow" size="mini">更新发票号</el-button>
      <el-input size="mini" v-show="dialogFormVisible||refundVisible" v-model="invoiceNo" placeholder="发票号"
                style="width: 200px;margin-left:30px" class="filter-item"></el-input>
      <el-button v-show="dialogFormVisible" size="mini">更新发票号</el-button>
      <el-button style="float:right;margin-left:20px" type="text" size="medium" @click="reprint"><i
        class="el-icon-circle-plus"/>重打
      </el-button>
      <el-button style="float:right;margin-left:20px" type="text" size="medium" @click="selectPatientByIdNo"><i
        class="el-icon-upload"/>读卡
      </el-button>
      <el-button style="float:right;margin-left:20px" type="text" size="medium" @click="guahao"><i
        class="el-icon-circle-plus"/>挂号
      </el-button>
    </aside>
    <div>
      <transition name=".el-fade-in-linear">
        <el-form v-if="isshow" label-position="left" :model="dmsRegistrationParam" label-width="68px" :inline="true"
                 class="demo-form-inline"
                 style="padding:0 0 0 100px;border-style: solid;border-width: 0px 0px 1px 0px;border-color:#C0C0C0;">
          ...
          <el-form-item style="width:300px" label="性别">
            <el-select placeholder="性别" v-model="dmsRegistrationParam.gender" style="width:100px">
              <el-option v-for="item in [{label:'男',value:0},{label:'女',value:1}]" :key="item.value" :label="item.label"
                         :value="item.value"/>
            </el-select>
          </el-form-item>
          ...
          <el-form-item style="width:300px" label="挂号科室">
            <el-select placeholder="挂号科室" filterable @change="chooseRegister" v-model="dmsRegistrationParam.deptId"
                       style="width:180px">
              <el-option v-for="item in alldept" :key="item.id" :label="item.name" :value="item.id"/>
            </el-select>
          </el-form-item>
          <el-form-item style="width:300px" label="挂号级别">
            <el-select placeholder="挂号级别" filterable v-model="dmsRegistrationParam.RegisterRnak"
                       @change="chooseRegister" style="width:180px">
          ...
          <el-form-item style="width:300px" label="午别">
            <el-select placeholder="默认" :disabled="regdisabled" style="width:100px" @change="choosenoon"
          ...
          <el-form-item style="width:300px" label="看诊医生">
            <el-select style="width:180px" placeholder="默认" v-model="dmsRegistrationParam.skdId"
          ...
          <el-form-item style="width:300px" label="应收金额">
            <el-input placeholder="应收金额" style="width:180px;color:black" disabled
                      v-model="dmsRegistrationParam.amount"></el-input>
          </el-form-item>
    ...
    <div v-if="!dialogFormVisible&&!isshow&&!refundVisible">
      ...
      <el-input placeholder="病历号/姓名" v-model="queryRegister.medicalRecordNo"
                style="margin-left:20px;width:180px"></el-input>
      <el-button @click="listRegisteredPatient">
        <svg-icon icon-class="search"/>
      </el-button>
      <el-button style="float:right;margin-right:30px" type="text" @click="refresh" size="medium"><i
        class="el-icon-refresh"/>刷新
      </el-button>
      <el-table border
                :data="RegisterList.filter(data => !queryRegister.medicalRecordNo || data.patientName.toLowerCase().includes(queryRegister.medicalRecordNo.toLowerCase())||data.medicalRecordNo.toLowerCase().includes(queryRegister.medicalRecordNo.toLowerCase()))"
                highlight-current-row style="width: 100%;margin-top:20px" height="400">
        <el-table-column
          fixed="left"
          label="操作"
          width="160" align="center">
          <template slot-scope="scope">
            <el-button v-if="scope.row.registrationStatus=='1'" plain size="mini" @click="tuihao(scope.row)">退号
            </el-button>
            <el-button v-if="scope.row.registrationStatus=='1'" plain size="mini" @click="supprint(scope.row)">补打
            </el-button>
            <el-button v-if="scope.row.registrationStatus=='2'" plain size="mini" @click="handlepay(scope.row)">缴费
            </el-button>
            <el-button v-if="scope.row.registrationStatus=='2'" plain size="mini"
                       @click="refundRegistrationCharge(scope.row)">退费
            </el-button>
    ...
    <div v-show="dialogFormVisible&&!refundVisible" style="text-align:center">
      <el-form :model="onepatient" label-width="80px" label-position="left" :inline="true">
        <el-form-item label="病历号" prop="id" label-width="60px">
          <el-tag style="width:200px" type="info" size="large" placeholder="病历号">{{ onepatient.medicalRecordNo }}
          </el-tag>
        </el-form-item>
        ...
        <el-form-item label="看诊日期" prop="birth" label-width="70px">
          <el-tag style="width:140px" type="info" size="large" placeholder="病历号">{{ onepatient.attendanceDate }}
          </el-tag>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="退号" :visible.sync="refundregistVisible" width="400px">
      <el-form :model="refundregist">
        ...
        <el-form-item>
          <el-button style="float:right;margin-left:10px" type="danger" @click="refundregistVisible = false">取消
          </el-button>
    ...
    <el-dialog title="重打" :visible.sync="reprintregistVisible" width="400px">
      <el-form>
        ...
        <el-form-item>
          <el-button style="float:right;margin-left:10px" type="danger" @click="reprintregistVisible = false">取消
          </el-button>
    ...
    <el-dialog title="补打" :visible.sync="suppregistVisible" width="400px">
      <el-form :model="refundregist">
        ...
        <el-form-item>
          <el-button style="float:right;margin-left:10px" type="danger" @click="suppregistVisible = false">取消
          </el-button>
