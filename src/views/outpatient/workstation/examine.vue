<template>
  ...
  <el-aside :width="mainwidth" style="background:white;padding:0 10px 0 0">
    ...
    align="center"
      ...
      show-overflow-tooltip>
      <template slot-scope="scope">
        <el-button type="text" v-if="scope.row.status===0" @click="demand(scope.row)">检验要求</el-button>
  ...
  <el-main width="50%" v-show="isclose" style="border-style: dotted;border-width: 0px 0px 0px 1px;border-color:#C0C0C0;margin-top:-12px">
     ...
     <el-tab-pane label="常用检验项" name="second">
  ...
  <el-dialog title="检验目录" :visible.sync="dialogTableVisible" top="50px">
    <div style="height:450px">
    <span>搜索检验</span>
    <el-input style="width:200px" v-model="search" placeholder="搜索检验"></el-input>
    <el-table height="400px" @row-click="selectCheck" highlight-current-row :data="checkList.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase())||data.code.toLowerCase().includes(search.toLowerCase()))" style="margin-top:20px">
      <el-table-column property="name" label="项目名" width="320px"></el-table-column>
      <el-table-column property="code" label="项目编码"></el-table-column>
      <el-table-column property="price" label="价格"></el-table-column>
    </el-table>
    </div>
  </el-dialog>
  <el-dialog title="填写检验要求" :visible.sync="demandVisible">
    <div style="height:260px">
      <el-form inline label-width="100px" >
        ...
        <el-form-item label="检验部位"><el-input placeholder=""  v-model="check.checkParts"></el-input></el-form-item>
      </el-form>
...
export default {
  ...
  methods:{
    ...
    addModel(val){
      this.$confirm('是否确定将 模板:'+val.name+' 中的内容导入该患者的检验中', '导入模板', {
    ...
    selectCheck(val){
      val.status=0
      this.$confirm('是否添加 '+val.name+' 到该患者?', '添加检验', {
          confirmButtonText: '确认',
          cancelButtonText: '取消',
          type: 'success'
        }).then(()=>{
